...
```text
As a developer I need help getting these tasks done. Read the ~/Nextcloud/Tasks/tuxedo/todos/development/todo.txt file and help me pick one task and start getting things done.
```
...
If the user gives an explicit path, use it first. Support references such as:
...
When the selected task requires multiple implementation steps, create a Claude Code session todo list for the current execution plan. Keep it separate from the user's `todo.txt` file; the Claude Code session todo list tracks current execution, while `todo.txt` tracks durable user progress.
